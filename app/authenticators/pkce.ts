...
export default class PKCEAuthenticator extends BaseAuthenticator {
   ...
   * @private
   */
  private upcomingRefresh: EmberRunTimer | undefined;
  ...
  async authenticate(code: string, state: string): Promise<ModifiedAuthTokenResponse> {
    ...
    if (await this.isValidState(state)) {
   ...
   */
  // eslint-disable-next-line prettier/prettier
  private async refresh(refresh_token: string /*, retryCount = 0 */): Promise<ModifiedAuthTokenResponse> {
    const span = startSpan({
      op: 'auth.refresh',
   ...
   */
  // eslint-disable-next-line prettier/prettier
  private async scheduleRefresh(expires_in: number, refresh_token: string): Promise<void> {
    startSpan({
   ...
   * @return {Boolean}
   */
  private async isValidState(state?: string): Promise<boolean> {
   ...
   */
  // eslint-disable-next-line prettier/prettier
  private async getUserinfo(accessToken: string): Promise<AuthUserInfo> {
    startSpan({
      op: 'auth.user_info',
   ...
   */
  // eslint-disable-next-line prettier/prettier
  private async post(url: string, params = {}): Promise<HttpResponse | void> {
    const headers = {
      Accept: 'application/json',
