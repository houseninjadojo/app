...
export interface Notification {
   ...
   * state
   *
   * @options delivered, received, scheduled
   *
   * @type local, remote
   * @platform ios, android
   */
...
export class Notification implements Notification {
  ...
  constructor(
    ...
    const t = type ?? Notification.typeFor(notification);
    this.state = state;
    this.type = t;
    this.id = notification.id;
    Object.assign(this, notification);

    if (t === NotificationType.Branch) {
      this.link = this.data.deeplink_path;
    }
  }
  ...
  static isLocal(n: NotificationSchema): boolean {
    return typeof n.id === 'number';
  }

  static isRemote(n: NotificationSchema): boolean {
    return typeof n.id === 'string';
  }

  static isIntercom(n: NotificationSchema): boolean {
    return isPresent(n.data?.intercom_push_type);
  }

  static isBranch(n: NotificationSchema): boolean {
    return isPresent(n.data?.branch);
  }

  static typeFor(n: NotificationSchema): NotificationType {
    switch (true) {
      ...
      case Notification.isBranch(n):
        return NotificationType.Branch;
      default:
